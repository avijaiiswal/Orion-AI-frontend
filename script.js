...
// Display info for each plan tier. Keys match the `status` field
// returned by the backend (user_subscriptions.status).
const PLAN_TIERS = {
    ...
    ultimate:{ name: "Yearly",  sub: "Active subscription" }, // backend uses 'ultimate' for the yearly tier
...
function updateSidebarPlan() {
    ...
    const subEl  = document.getElementById("currentPlanSub");
    const btnEl  = document.getElementById("currentPlanBtn");
    if (!nameEl || !subEl || !btnEl) return;
    ...
    btnEl.classList.toggle("plan-tier-free", status === "free");
}
